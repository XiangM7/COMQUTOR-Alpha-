#!/usr/bin/env python3
"""QA Closure v0.1.2 -- Item 2 Blind Holdout #3, Phase 5: run the CURRENT,
frozen, unmodified production Alpha Mapper (map_claim_to_alpha) against
every frozen Holdout #3 row, exercising the real LLM-primary semantic path
(classifier_enabled=True, a real Week2LLMGateway/Provider client -- never a
simulated/deterministic stand-in for the Alpha LLM, per task spec section
11).

For each row, ALSO computes the deterministic-only counterfactual (Section
17): the same map_claim_to_alpha call with classifier_enabled=False. This
is a pure, zero-cost, offline local computation -- no Provider call, no
gateway -- giving the exact "what would the old deterministic-fallback path
alone have produced" answer directly from the real production function,
never reconstructed/guessed from diagnostic fields.

Every diagnostic field captured here (candidate_scores, eligible_candidates,
classifier status, alpha_match_method, etc.) is for POST-evaluation audit
only and is never shown to the independent reviewer (a separate script).
"""
...
OUTPUT_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout3_system_alpha.json"
PROVIDER_AUDIT_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout3_provider_audit.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    # every row genuinely gets an LLM attempt rather than silently
    # exhausting a shared budget partway through.
    CHUNK_SIZE = 100
    ...
    for chunk_start in range(0, n, CHUNK_SIZE):
        ...
        gateway = Week2LLMGateway(
            model, run_id=f"item2-blind-holdout3-alpha-chunk{chunk_start // CHUNK_SIZE}",
        ...
        for row in chunk:
            ...
            # Zero-cost, offline, no Provider call: the same real production
            # function with the LLM tier disabled -- the deterministic-only
            # counterfactual (task spec section 17).
            deterministic_result = map_claim_to_alpha(record, taxonomy, classifier_enabled=False, llm_gateway=None)
            ...
            results[row["sample_id"]] = {
                ...
                "deterministic_counterfactual_matched_alpha_id": deterministic_result.get("matched_alpha"),
                "deterministic_counterfactual_match_status": deterministic_result.get("match_status"),
                "deterministic_counterfactual_score": deterministic_result.get("score"),
                "row_elapsed_seconds": round(row_elapsed, 3),
            }
        total_calls += gateway.call_count

    elapsed_total = time.monotonic() - started_total
    call_count_final = gateway.call_count

    OUTPUT_PATH.write_text(json.dumps(results, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    fallback_count = sum(1 for r in results.values() if r["alpha_match_method"] == "deterministic_fallback")
    llm_count = sum(1 for r in results.values() if r["alpha_match_method"] == "llm")
    provider_audit = {
        "phase": "phase5_production_alpha_inference",
        "provider": profile.llm_provider,
        "model_name": profile.quick_think_llm,
        "temperature": "production default (unmodified)",
        "prompt_version": "week2.alpha_classifier.v2",
        "rows_total": n,
        ...
        "retries_used": 0,
        "alpha_match_method_llm_count": llm_count,
        "alpha_match_method_deterministic_fallback_count": fallback_count,
        "elapsed_seconds": round(elapsed_total, 2),
    }
    existing_audit = json.loads(PROVIDER_AUDIT_PATH.read_text(encoding="utf-8")) if PROVIDER_AUDIT_PATH.exists() else {}
    existing_audit["phase5_production_alpha_inference"] = provider_audit
