#!/usr/bin/env python3
"""QA Closure v0.1.2 -- Item 2 Blind Holdout #3, Phase 6: independent blind
Alpha reviewer.

A genuinely separate Provider call sequence from Phase 5 (production Alpha
inference) -- its own gateway/client construction, no shared conversation
context, no cached prior answers, no shared prompt text (task spec section
10). The reviewer sees ONLY sample_id/ticker/claim/evidence and the FULL
canonical 10-Alpha taxonomy -- never target_alpha_id (the frozen Holdout #3
CSV does not even contain one), never current_matched_alpha, never any
production score/candidate/rank/eligibility/AI-gate/fallback/B1 field (task
spec section 8).

The reviewer prompt below is frozen before this script is ever run against
real data and is not altered after seeing any result (task spec section 10).
"""
...
OUTPUT_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout3_alpha_review.json"
PROVIDER_AUDIT_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout3_provider_audit.json"

BATCH_SIZE = 10
MAX_ATTEMPTS_PER_BATCH = 3
TIMEOUT_SECONDS = 120.0
REVIEW_SOURCE = "independent_llm_blind_holdout3"
REVIEWER_PROMPT_VERSION = "item2_blind_holdout3.independent_alpha_reviewer.v1"

VALID_CONFIDENCE = frozenset({"high", "medium", "low"})

# Frozen BEFORE any reviewer inference is run; never altered after seeing a
# result (task spec section 10).
...
thesis expressed in the Evidence, not surface word overlap. Return expected_alpha_id = "NONE" only when no \
...
def main(argv: list[str] | None = None) -> int:
    ...
    # shared with Phase 5's own gateway/client instance.
    ...
    call_summary = {
        "phase": "phase6_independent_alpha_reviewer",
    ...
    existing_audit["phase6_independent_alpha_reviewer"] = call_summary
