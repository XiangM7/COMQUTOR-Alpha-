#!/usr/bin/env python3
"""Execute the frozen four-slot v4.1 final Re-Canary exactly once.

This audit entrypoint is intentionally one-shot, sequential, zero-retry and
Legacy-authority preserving.  A successful 4/4 result only authorizes the
separate local cutover edits performed after this script finishes; this file
never changes production authority itself.
"""
...
CONTRACT_PATH = REPO_ROOT / (
    ...
    "phase1_v4_1_final_recanary_execution_contract.json"
)
RESULT_PATH = REPO_ROOT / (
    "docs/audit_artifacts/phase1_master/"
    "phase1_v4_1_final_recanary_runtime_result.json"
)
APPROVAL_ENV = "COMQUTOR_PHASE1_V4_1_FINAL_RECANARY_APPROVED"
EGRESS_ENV = "COMQUTOR_PHASE1_V4_1_FINAL_RECANARY_DATA_EGRESS_APPROVED"
...
def _load_contract() -> dict[str, Any]:
    ...
    checks = (
        ...
        contract.get("prompt_version") == STRUCTURED_OUTPUT_SHADOW_PROMPT_VERSION_V4_1,
        contract.get("prompt_sha256") == STRUCTURED_OUTPUT_SHADOW_PROMPT_V4_1_SHA256,
        contract.get("production_authority_before_execution") == "LEGACY_ADAPTER",
    )
    if not all(checks):
        raise RuntimeError("FINAL_RECANARY_EXECUTION_CONTRACT_INVALID")
    expected_families = ["fundamental", "news", "sentiment", "technical"]
    if [item.get("family") for item in contract["slots"]] != expected_families:
        raise RuntimeError("FINAL_RECANARY_SLOT_ORDER_INVALID")
    return contract


def _payload_from_frozen_sources(
    ...
    for expected_slot, slot in enumerate(contract["slots"], start=1):
        if slot.get("slot") != expected_slot:
            raise RuntimeError("FINAL_RECANARY_SLOT_ORDER_DRIFT")
        record = by_id.get(str(slot.get("source_agent_output_id")))
        if record is None:
            raise RuntimeError("FINAL_RECANARY_SOURCE_REPORT_MISSING")
        source_report = str(record.get("raw_output") or "")
        if _sha256_text(source_report) != slot.get("source_report_sha256"):
            raise RuntimeError("FINAL_RECANARY_SOURCE_REPORT_HASH_MISMATCH")
        if record.get("agent") != slot.get("agent"):
            raise RuntimeError("FINAL_RECANARY_SOURCE_AGENT_MISMATCH")
        selected.append(
            {
                "agent": record["agent"],
                "tradingagents_agent": record.get("tradingagents_agent")
                or record["agent"],
                "source_field": record.get("source_field") or "recanary_source_report",
...
def _summarize_resolution(diagnostics: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in diagnostics:
        ...
        if int(item.get("global_exact_match_count") or 0) > 1:
            duplicate_count += 1
    ...
    return {
        ...
        "duplicate_global_match_items": duplicate_count,
    }


class FinalRecanaryExecutor:
    def __init__(
        self,
        *,
        contract: dict[str, Any],
        ...
        if (
            profile.llm_provider != self.provider
            or profile.quick_think_llm != self.model
        ):
            raise RuntimeError("FINAL_RECANARY_PROVIDER_IDENTITY_DRIFT")
        ...
        if (
            ...
            raise RuntimeError("FINAL_RECANARY_EXECUTION_POLICY_DRIFT")
    ...
    def _persist_slot(
        ...
        *,
        ...
        if attempt_count > 1:
            raise RuntimeError("FINAL_RECANARY_PROVIDER_ATTEMPT_CAP_BREACHED")
        retry_count = int(getattr(invocation, "retry_count", 0) or 0)
        if retry_count != 0:
            raise RuntimeError("FINAL_RECANARY_RETRY_OCCURRED")
        trace = getattr(invocation, "trace_handle", None)
        validation = (bundle or {}).get("validation_summary") or {}
        current_legacy = _without_times(
            json.loads(
                (self.shadow_run_dir / "structured_agent_outputs.json").read_text(
                    encoding="utf-8"
                )
            )
        )
        result = {
            "schema_version": "comqutor.phase1_v4_1.final_recanary_slot.v1",
            ...
            "legacy_isolation_pre_downstream": (
                current_legacy == self.legacy_structured_snapshot
            ),
            "unexpected_error_type": (
                type(unexpected_error).__name__ if unexpected_error else None
            ),
        ...
        progress = self.state["final_v4_1_recanary_progress"]
        ...
        _commit_budget(
            ...
            1,
            attempt_count,
            f"final_v4_1_recanary:{self.contract['canary_id']}:{slot['family']}",
            global_event={
                "schema_version": "comqutor.phase1_global_provider_call_ledger.v1",
                "event_id": (
                    f"phase1_master:final_v4_1_recanary:{self.contract['canary_id']}:"
                    f"{slot['source_agent_output_id']}:"
                    f"{STRUCTURED_OUTPUT_SHADOW_PROMPT_VERSION_V4_1}"
                ),
                "recorded_at": ended_at,
                "source_phase": "PHASE_1_MASTER",
                "stage": "FINAL_V4_1_RECANARY",
    ...
    def execute(self, context: Any) -> ShadowAttemptResult:
        if len(self.results) >= 4:
            raise RuntimeError("FINAL_RECANARY_LOGICAL_CALL_CAP_BREACHED")
        slot = self.slot_by_agent.get(str(context.agent))
        if slot is None or int(slot["slot"]) != len(self.results) + 1:
            raise RuntimeError("FINAL_RECANARY_RUNTIME_SLOT_ORDER_DRIFT")
        if _sha256_text(context.source_report) != slot["source_report_sha256"]:
            raise RuntimeError("FINAL_RECANARY_RUNTIME_SOURCE_HASH_MISMATCH")
        ...
        try:
            bundle, invoker, candidate_segments, diagnostics, forensic = (
                self.provider_module.run_real_provider_shadow_smoke_for_report_v4_1(
        ...
        candidate_manifest = build_candidate_manifest_v4_1(
            source_report=context.source_report,
            run_id=context.run_id,
            ticker=context.ticker,
            agent=context.agent,
        ...
        if bundle is None:
            raise RuntimeError("FINAL_RECANARY_BUNDLE_MISSING")
...
def _initialize_state(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or state.get("cumulative_provider_attempts", 0) + 4
        > HARD_CAP_PROVIDER_ATTEMPTS
    ):
        raise RuntimeError("FINAL_RECANARY_START_STATE_OR_BUDGET_INVALID")
    state["history"].append(
        {
            "at": _now(),
            "from_state": state.get("current_state"),
            "to_state": "FINAL_V4_1_RECANARY",
            "detail": {
    ...
    state["current_state"] = "FINAL_V4_1_RECANARY"
    state["final_v4_1_recanary_progress"] = {
...
def run_recanary() -> dict[str, Any]:
    contract = _load_contract()
    if RESULT_PATH.exists():
        raise RuntimeError("FINAL_RECANARY_ALREADY_EXECUTED")
    if os.environ.get(APPROVAL_ENV, "").strip().lower() != "true":
        raise RuntimeError("FINAL_RECANARY_APPROVAL_MISSING")
    if os.environ.get(EGRESS_ENV, "").strip().lower() != "true":
        raise RuntimeError("FINAL_RECANARY_DATA_EGRESS_APPROVAL_MISSING")
    if not os.environ.get("ANTHROPIC_API_KEY"):
        raise RuntimeError("FINAL_RECANARY_ANTHROPIC_CREDENTIAL_MISSING")

    payload, source_hash_before = _payload_from_frozen_sources(contract)
    artifact_root = REPO_ROOT / str(contract["artifact_root"])
    if artifact_root.exists():
        raise RuntimeError("FINAL_RECANARY_ARTIFACT_ROOT_ALREADY_EXISTS")
    ...
    executor = FinalRecanaryExecutor(
        contract=contract,
        state=state,
    ...
    for raw_result in executor.results:
        ...
        accepted = (
            sidecar.get("accepted") is True
            and int(sidecar.get("shadow_claim_count") or 0) > 0
        )
        accepted_count += int(accepted)
        diagnostics = raw_result["resolution_diagnostics"]
        duplicate_branch = duplicate_branch or any(
            int(item.get("global_exact_match_count") or 0) > 1
            for item in diagnostics
        )
        provenance_pass = bool(diagnostics) and all(
            item.get("resolution_status") in RESOLUTION_STATUSES_LOCATED
            and item.get("source_report_sha256") == raw_result["source_report_sha256"]
            for item in diagnostics
        )
        candidate_binding_pass = provenance_pass and all(
            int(item.get("global_exact_match_count") or 0) <= 1
            or (
                item.get("resolution_status")
                == RESOLUTION_DISAMBIGUATED_VIA_CANDIDATE
                and bool(item.get("selected_candidate_id"))
                and int(item.get("candidate_local_match_count") or 0) == 1
            )
            for item in diagnostics
        )
        identity_pass = all(
            (
                ...
                sidecar.get("prompt_version") == STRUCTURED_OUTPUT_SHADOW_PROMPT_VERSION_V4_1,
                sidecar.get("prompt_sha256") == STRUCTURED_OUTPUT_SHADOW_PROMPT_V4_1_SHA256,
            )
        )
        sidecar_persisted = bool(sidecar) and (
            sidecar.get("candidate_manifest") == raw_result["candidate_manifest"]
        )
        ...
        finalized = {
            **raw_result,
            ...
            "candidate_binding": "PASS" if candidate_binding_pass else "FAIL",
            ...
            "legacy_isolation": (
                "PASS" if authoritative_equal and response_equal else "FAIL"
            ),
        ...
        _write_json(
            artifact_root
            / "slot_results"
            / f"{int(raw_result['logical_call_number']):02d}-{raw_result['family']}.json",
            finalized,
        )
    ...
    status = (
        ...
        and source_hash_before
        == _sha256_file(REPO_ROOT / str(contract["source_artifact"]))
        and unresolved_admitted == 0
        and identity_failure_admitted == 0
        else "FAIL"
    )
    result = {
        "schema_version": "comqutor.phase1_v4_1.final_recanary_runtime_result.v1",
        "canary_id": contract["canary_id"],
        "status": status,
        "production_authority": "LEGACY_ADAPTER",
        ...
        "duplicate_quote_branch_exercised": duplicate_branch,
        ...
        "source_artifact_unchanged": source_hash_before
        == _sha256_file(REPO_ROOT / str(contract["source_artifact"])),
    ...
    _write_json(artifact_root / "recanary_runtime_result.json", result)
    _write_json(RESULT_PATH, result)
    progress = state["final_v4_1_recanary_progress"]
    progress.update(
        {
            "status": status,
    ...
    state["next_action"] = (
        "FINAL_V4_1_RECANARY_PASS; execute authorized local production cutover with zero further Provider calls."
        if status == "PASS"
        else "FINAL_V4_1_RECANARY_FAIL; retain Legacy authority and stop Section 5.1."
    )
    save_state(state)
    return result
...
def main() -> int:
    ...
    parser.add_argument("--execute-provider-recanary", action="store_true")
    args = parser.parse_args()
    if not args.execute_provider_recanary:
        print("FINAL_RECANARY_NOT_EXECUTED: pass --execute-provider-recanary")
        return 2
    try:
        result = run_recanary()
    except Exception as exc:
        print(f"FINAL_V4_1_RECANARY_EXECUTION_ERROR={type(exc).__name__}")
        print(str(exc))
        return 1
    print(f"FINAL_V4_1_RECANARY={result['status']}")
