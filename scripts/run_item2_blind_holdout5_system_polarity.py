#!/usr/bin/env python3
"""Blind Holdout #5, Phase 2 (system side): run the CURRENT, frozen,
...
against the polarity-evaluable subset of H5 (rows where the frozen review's
human_expected_alpha_id != "NONE"), using that reviewed value as each row's
target Alpha. Same invocation pattern as every prior holdout's B1 system
evaluation -- only the target-Alpha values and the row set differ.

Does NOT change any B1 production rule. Real Provider calls, production
BATCH_SIZE default (unmodified). Reads the FROZEN review snapshot
(item2_blind_holdout5_review_frozen.csv), never the mutable working file.
"""
...
OUTPUT_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout5_system_polarity.json"
...
def _load_evaluable_rows() -> list[dict[str, Any]]:
    ...
    with REVIEW_FROZEN_PATH.open(encoding="utf-8", newline="") as f:
        review_by_id = {r["sample_id"]: r for r in csv.DictReader(f)}

    rows = []
    for sid, frozen in frozen_by_id.items():
        rev = review_by_id[sid]
        target_alpha_id = rev["human_expected_alpha_id"]
        if target_alpha_id == "NONE" or not target_alpha_id:
            continue
        rows.append(
            {
...
def main(argv: list[str] | None = None) -> int:
    ...
    n = len(rows)
    print(f"Polarity-evaluable rows (reviewed expected_alpha_id != NONE): {n} / 200")
    ...
    gateway = Week2LLMGateway(
        model, run_id="item2-blind-holdout5-polarity", output_root=str(REPO_ROOT / "outputs" / "runs"),
    ...
    for match in matches:
        ...
        out_results[match["_sample_id"]] = {
            ...
            "system_polarity": candidate.get("evidence_stance"),
            "system_polarity_counter_alpha_id": candidate.get("counter_alpha_id"),
            "method": candidate.get("stance_method"),
            "fallback_reason": candidate.get("stance_fallback_reason"),
    ...
    call_log = {
        "phase": "holdout5_production_polarity_inference",
        "provider": profile.llm_provider, "model_name": profile.quick_think_llm,
    ...
    existing_audit["holdout5_production_polarity_inference"] = call_log
