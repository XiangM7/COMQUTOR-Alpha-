#!/usr/bin/env python3
"""QA Closure v0.1.2 -- Item 2 Blind Holdout #2, Phase 5: run the CURRENT,
exact production Alpha Mapper (comqutor_alpha.structure_engine.alpha_mapper.
map_claim_to_alpha) fresh against each of the 141 frozen Holdout #2 rows'
...
from the holdout CSV's own trimmed columns). Same methodology as Holdout #1's
run_item2_blind_holdout_alpha_mapper.py.
...
OUTPUT_CSV_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout2_system_alpha.csv"

CSV_FIELDS = [
    "sample_id", "claim_id", "target_alpha_id", "matched_alpha_id", "matched_alpha_name", "match_score",
    "match_status", "candidate_ranking", "rejection_reason_for_matched", "ai_gate_applicable",
...
def main() -> int:
    ...
    assert len(rows) == 141, f"expected 141 frozen Holdout #2 rows, found {len(rows)}"
