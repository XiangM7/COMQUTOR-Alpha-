#!/usr/bin/env python3
"""QA Closure v0.1.2 -- Item 2 Blind Holdout #4, Phase 2 (system side): run
the REAL, CURRENT, frozen production Pure-LLM Alpha Mapper (map_claim_to_
alpha) against every one of the 200 frozen Holdout #4 rows, exercising the
actual production Alpha semantic path (classifier_enabled=True, a real
Week2LLMGateway/Provider client -- never a simulated/deterministic stand-in
for the Alpha LLM).

A genuinely separate Provider call sequence/session from the independent
blind reviewer phase (scripts/run_item2_blind_holdout4_alpha_review.py) --
own gateway/client construction, own run_id namespace, no shared state.

Chunks across multiple Week2LLMGateway instances from the very first run
(CHUNK_SIZE=100, since Week2LLMGateway.MAX_SERVER_CALLS=100 silently capped
Holdout #3's very first run at 100/152 attempts before that bug was caught
-- task spec section 16's own explicit reminder). 200 rows -> exactly 2
chunks.
...
POST-evaluation audit only and is never shown to the independent reviewer.
"""
...
OUTPUT_PATH = REPO_ROOT / "docs" / "audit_artifacts" / "item2_blind_holdout4_system_alpha.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    n = len(rows)
    print(f"Loaded {n} frozen Holdout #4 rows.")
    ...
    for chunk_start in range(0, n, CHUNK_SIZE):
        ...
        gateway = Week2LLMGateway(
            model, run_id=f"item2-blind-holdout4-alpha-chunk{chunk_start // CHUNK_SIZE}",
        ...
        for row in chunk:
            ...
            # Pure-LLM authority: one call yields both the semantic result
            # AND the deterministic counterfactual (deterministic_top_alpha/
            # deterministic_match_status) in the same result dict.
    ...
    provider_audit = {
        "phase": "holdout4_production_alpha_inference",
    ...
    existing_audit["holdout4_production_alpha_inference"] = provider_audit
