"""Provider-zero Phase 1 human-review packet construction -- v3 (Phase 1
Master evidence-alignment fix).

This is an ADDITIVE sibling of ``phase1_human_review_packet.py`` (the v2
packet builder): neither that module nor its ``phase1_human_review_v2.csv/
.html`` output is modified, overwritten, or deleted. This module reads
already-persisted accepted v3 Shadow artifacts (``prompt_version ==
"structured_adapter.claim_extraction_shadow.v3"``) and renders a self-
contained, one-row-per-claim review CSV plus a local HTML view, with
exactly the same field-level rigor as the v2 builder: every reviewer-facing
value (claim text, evidence quote, surrounding context, entities, factors,
direction, confidence) is read verbatim from the persisted, already-
validated canonical Shadow bundle -- never recomputed, paraphrased, or
inferred here. It has no Provider, gateway, cache, network, Redis, or
database dependency.
"""
...
PACKET_SCHEMA_VERSION = "comqutor.phase1.human_review_packet_v3.v1"
VERIFICATION_SCHEMA_VERSION = "comqutor.phase1.human_review_packet_v3_verification.v1"
PROVENANCE_FAILURE = "HUMAN_REVIEW_PACKET_V3_PROVENANCE_FAILURE"
COVERAGE_FAILURE = "HUMAN_REVIEW_PACKET_V3_COVERAGE_FAILURE"

REQUIRED_PROMPT_VERSION = "structured_adapter.claim_extraction_shadow.v3"
...
# free-text provenance the v2 packet already records.
REVIEW_FIELDS = (
    "semantic_fidelity",
    "claim_boundary",
...
ALLOWED_REVIEW_VALUES = {
    ...
    # Deliberately identical to phase1_human_review_packet.py (v2)'s
    # vocabulary -- not just for consistency, but so scripts/run_phase1_
    # master.py's existing, UNMODIFIED validate_human_review_csv /
    # _validate_human_review_v2_csv (which import their allowed-values
    # table from the v2 module) validate a completed v3 CSV correctly with
    # no v3-specific branching required. A real "exact quote, wrong
    # section" error is still fully expressible as FABRICATED_EVIDENCE (or
    # OTHER, with detail in notes) -- see render_review_instructions.
    "critical_error_type": (
        "NONE",
        "NEGATION_REVERSAL",
        "ATTRIBUTION_LOSS",
...
def collect_review_rows(
    ...
    *,
    ...
    """Collect and validate exactly one self-contained row per accepted v3
    Claim. Fails closed (raises :class:`HumanReviewPacketError`) unless the
    persisted evaluation was genuinely run under the frozen v3 prompt
    identity -- this module must never silently render a v1 or v2
    evaluation's claims under the v3 packet's column contract."""
    ...
    if (
        ...
        raise HumanReviewPacketError(
            PROVENANCE_FAILURE,
            "evaluation_metadata_prompt_v3_identity_invalid",
        )
    ...
    for report_dir in report_dirs:
        ...
        # v3's deterministic EvidenceQuoteResolver can legitimately reject a
        # whole report (a quote failed to resolve -- see
        # structured_output_shadow_v3.resolve_supporting_quote) even though
        # the Provider responded successfully; unlike v2 (where a real
        # response was always admitted), that is an EXPECTED, correct
        # outcome for v3, not a provenance failure. A cleanly rejected
        # report contributes zero rows -- there is no accepted evidence to
        # review -- but must not block the packet for the other reports.
        # Anything else (a Provider error, timeout, or any other
        # unexpected state) still fails closed exactly as before.
        is_accepted = (
...
def render_review_html(
    ...
    parts = [
        ...
        "<title>Phase 1 Human Review Packet v3</title>",
        """<style>
...
.allowed{font-size:.78rem;color:#586574}.span{margin:1rem 0;padding:.8rem;border:1px dashed #9aa7b5;border-radius:7px}.nav a{color:#dbeafe;margin-right:.7rem}code{white-space:pre-wrap}@media print{header.top{position:static}.claim{break-inside:avoid;box-shadow:none}}
        ...
        '<header class="top"><strong>Phase 1 Human Review Packet v3 (evidence-alignment fix)</strong>',
        '<div class="nav">',
        *(
        ...
        "report's proposal was rejected (never partially admitted). That is expected, "
        "correct v3 behavior, not a defect; see shadow_exact_replay_audit.json and each "
        "rejected report's validation_report.json for the reason codes.</p>",
...
def render_review_instructions(metrics: Mapping[str, Any]) -> str:
    ...
    return f"""# Phase 1 Human Review Instructions — packet v3 (evidence-alignment fix)

This packet contains one row for every persisted accepted v3 Shadow Claim: **{metrics['total_claims']} claims across {accepted_reports} of {metrics['total_reports']} reports** ({rejected_reports} report(s) were rejected in full by the deterministic validator -- at least one proposed quote could not be uniquely located as an exact match in the report text -- and contribute zero rows; that is expected, correct v3 behavior, not a defect). Complete `phase1_human_review_v3.csv`; the HTML file is a local inspection aid. Do not edit claim, evidence, identity, context, or structured-semantic columns.

## Why this packet exists

The v2 evaluation showed Claim extraction works, but review found a systemic problem: a Claim could be correct while its bound evidence span was not actually the sentence that supports it. v3 changed the Provider contract so the model proposes verbatim quotes instead of numeric offsets, and a deterministic resolver locates each quote by exact string match only. **Exact-match provenance is not semantic correctness** -- the resolver proves the quoted text really appears in the report, never that it actually supports the claim. That judgment is exactly what `evidence_correctness` below is for.
...
def build_review_packet(
    ...
    *,
    ...
    """Build v3 artifacts atomically without mutating canonical inputs/state."""
    ...
    output_texts = {
        "phase1_human_review_v3.csv": csv_text,
        "phase1_human_review_v3.html": html_text,
        "phase1_human_review_v3_instructions.md": instructions_text,
    }
    ...
    csv_path = evaluation_dir / "phase1_human_review_v3.csv"
    html_path = evaluation_dir / "phase1_human_review_v3.html"
    instructions_path = evaluation_dir / "phase1_human_review_v3_instructions.md"
    ...
    verification = {
        ...
        "coverage": {
            **metrics,
            ...
            "review_v3_row_count": len(parsed_rows),
        },
        "checks": checks,
        "outputs": {
            "csv": str(csv_path),
        ...
        "semantic_quality": "UNPROVEN_V3",
        "human_review": "PENDING_V3",
    ...
    if _secret_findings({"phase1_human_review_packet_v3_verification.json": verification_text}):
        raise HumanReviewPacketError(
            PROVENANCE_FAILURE,
            "secret_pattern_detected_in_verification",
        )
    verification_path = evaluation_dir / "phase1_human_review_packet_v3_verification.json"
