"""Provider-zero Phase 1 human-review packet construction -- v4 (Phase 1
Master evidence-alignment fix, round 2: markdown-formatting-fidelity fix).

This is an ADDITIVE sibling of ``phase1_human_review_packet.py`` (v2) and
``phase1_human_review_packet_v3.py`` (v3): none of them, or their outputs,
are modified, overwritten, or deleted. This module reads already-persisted
v4 Shadow artifacts (``prompt_version ==
"structured_adapter.claim_extraction_shadow.v4"``) and renders a self-
contained, one-row-per-claim review CSV plus a local HTML view, with
exactly the same field-level rigor as the v2/v3 builders: every reviewer-
facing value (claim text, evidence quote, surrounding context, entities,
factors, direction, confidence) is read verbatim from the persisted,
already-validated canonical Shadow bundle -- never recomputed, paraphrased,
or inferred here. It has no Provider, gateway, cache, network, Redis, or
database dependency.

Byte-identical logic to the v3 packet builder -- including tolerating a
report that the deterministic EvidenceQuoteResolver legitimately rejected
in full (zero claim rows, not a provenance failure), a real behavior the
v3 evaluation exposed for the first time -- with only the prompt identity,
schema/output-filename constants, and this docstring changed.
"""
...
PACKET_SCHEMA_VERSION = "comqutor.phase1.human_review_packet_v4.v1"
VERIFICATION_SCHEMA_VERSION = "comqutor.phase1.human_review_packet_v4_verification.v1"
PROVENANCE_FAILURE = "HUMAN_REVIEW_PACKET_V4_PROVENANCE_FAILURE"
COVERAGE_FAILURE = "HUMAN_REVIEW_PACKET_V4_COVERAGE_FAILURE"

REQUIRED_PROMPT_VERSION = "structured_adapter.claim_extraction_shadow.v4"
...
# Field names match the Phase 1 Master evidence-alignment fix task
# specification exactly (semantic_fidelity / claim_boundary /
# evidence_correctness / entity_correctness / factor_correctness /
# direction_correctness / critical_error_type / overall_disposition /
# severity / reviewer / notes), plus review_timestamp for the same
# free-text provenance the v2/v3 packets already record.
REVIEW_FIELDS = (
    "semantic_fidelity",
    "claim_boundary",
...
# Deliberately identical to phase1_human_review_packet.py (v2) and
# phase1_human_review_packet_v3.py (v3)'s vocabulary -- not just for
# consistency, but so scripts/run_phase1_master.py's existing, UNMODIFIED
# validate_human_review_csv / _validate_human_review_v2_csv (which import
# their allowed-values table from the v2 module) validate a completed v4
# CSV correctly with no v4-specific branching required.
...
def collect_review_rows(
    ...
    *,
    ...
    """Collect and validate exactly one self-contained row per accepted v4
    Claim. Fails closed (raises :class:`HumanReviewPacketError`) unless the
    persisted evaluation was genuinely run under the frozen v4 prompt
    identity -- this module must never silently render a v1/v2/v3
    evaluation's claims under the v4 packet's column contract.

    A report the deterministic EvidenceQuoteResolver legitimately rejected
    in full (a quote failed to resolve -- see
    ``structured_output_shadow_v4.resolve_supporting_quote``) contributes
    zero rows but is not a provenance failure; only a genuinely anomalous
    report state (e.g. a Provider error or an inconsistent identity) fails
    closed."""
    ...
    if (
        ...
        or metadata.get("prompt_version") != REQUIRED_PROMPT_VERSION
        or not isinstance(metadata.get("prompt_sha256"), str)
        or not metadata.get("prompt_sha256")
    ):
        raise HumanReviewPacketError(
            PROVENANCE_FAILURE,
            "evaluation_metadata_prompt_v4_identity_invalid",
        )
    ...
    for report_dir in report_dirs:
        ...
        # v4's deterministic EvidenceQuoteResolver can legitimately reject a
        # whole report (a quote failed to resolve) even though the Provider
        # responded successfully -- that is an EXPECTED, correct outcome,
        # not a provenance failure. A cleanly rejected report contributes
        # zero rows -- there is no accepted evidence to review -- but must
        # not block the packet for the other reports. Anything else (a
        # Provider error, timeout, or any other unexpected state) still
        # fails closed exactly as before.
        is_accepted = (
            report_result.get("provider_status") == "success"
            and report_result.get("validation_status") == "accepted"
            and report_result.get("shadow_status") == "accepted"
            and report_result.get("validation_accepted") is True
            and validation.get("status") == "accepted"
            and validation.get("valid") is True
        )
        is_cleanly_rejected = (
            report_result.get("provider_status") == "success"
            and report_result.get("validation_status") == "rejected"
            and report_result.get("shadow_status") == "validation_rejected"
            and report_result.get("validation_accepted") is False
            and validation.get("status") == "validation_rejected"
            and validation.get("valid") is False
        )
        if not (is_accepted or is_cleanly_rejected):
            raise HumanReviewPacketError(
                PROVENANCE_FAILURE,
                f"report_not_accepted_or_cleanly_rejected:{report_dir}",
            )
    ...
    metrics = {
        ...
        "accepted_reports": len(report_dirs) - rejected_reports,
...
def render_review_html(
    ...
    parts = [
        ...
        "<title>Phase 1 Human Review Packet v4</title>",
        """<style>
...
.allowed{font-size:.78rem;color:#586574}.span{margin:1rem 0;padding:.8rem;border:1px dashed #9aa7b5;border-radius:7px}.nav a{color:#dbeafe;margin-right:.7rem}code{white-space:pre-wrap}@media print{header.top{position:static}.claim{break-inside:avoid;box-shadow:none}}
        ...
        '<header class="top"><strong>Phase 1 Human Review Packet v4 (markdown-formatting-fidelity fix)</strong>',
        '<div class="nav">',
        *(
        ...
        f"({_html_text(metrics.get('accepted_reports', metrics['total_reports']))} accepted, "
        f"{_html_text(metrics.get('rejected_reports', 0))} rejected by the deterministic "
        "validator -- see below) · "
        ...
        "<p>A rejected report contributes zero rows here -- at least one supporting_quote "
        "could not be located as a unique exact match in the report text, so the whole "
        "report's proposal was rejected (never partially admitted). v4 added an explicit "
        "instruction telling the model to preserve Markdown formatting characters (for "
        "example **bold**) exactly when quoting, after v3's real evaluation found that "
        "was the cause of 94% of its quote-resolution failures.</p>",
        "<p>Every evidence quote below was located in the persisted report by exact "
        "deterministic string match only (never fuzzy-matched or repaired) -- that "
        "proves PROVENANCE, not that the quote actually supports the claim. "
        "<strong>evidence_correctness is exactly the judgment this packet exists for.</strong> "
        "The CSV is the canonical review input. This HTML is a local, read-only "
        "inspection aid. Legacy content is comparison context, not gold.</p></section>",
    ]
    ...
    for row in rows:
        ...
        parts.extend(
            [
                ...
                '<div class="label">&gt;&gt;&gt; EXACT SUPPORTING EVIDENCE (verbatim, exact-match located)</div>',
...
def render_review_instructions(metrics: Mapping[str, Any]) -> str:
    ...
    accepted_reports = metrics.get("accepted_reports", metrics["total_reports"])
    rejected_reports = metrics.get("rejected_reports", 0)
    return f"""# Phase 1 Human Review Instructions — packet v4 (markdown-formatting-fidelity fix)

This packet contains one row for every persisted accepted v4 Shadow Claim: **{metrics['total_claims']} claims across {accepted_reports} of {metrics['total_reports']} reports** ({rejected_reports} report(s) were rejected in full by the deterministic validator and contribute zero rows; that is expected, correct behavior, not a defect). Complete `phase1_human_review_v4.csv`; the HTML file is a local inspection aid. Do not edit claim, evidence, identity, context, or structured-semantic columns.

## Why this packet exists

v3's real 24-report evaluation proved the EvidenceQuoteResolver design works (zero fabricated or unresolved quotes were ever admitted), but rejected 33.3% of reports outright. Root-cause analysis found 94% of the underlying quote-resolution failures traced to one mechanical cause: the model silently dropped or repositioned Markdown emphasis characters (`**bold**`) when copying a phrase, because v3's prompt never told it those characters are literal report content. v4 adds exactly one new instruction telling the model to preserve Markdown formatting exactly. **Exact-match provenance is still not semantic correctness** -- the resolver proves the quoted text really appears in the report, never that it actually supports the claim. That judgment is exactly what `evidence_correctness` below is for.
...
Does every quoted source span actually SUPPORT the Claim -- not merely appear verbatim in the report? `PARTIAL` means some, but not all, of the Claim is supported by the cited evidence. Watch specifically for a real, exact quote taken from the wrong part of the report (for example: evidence about a different metric, segment, or period that happens to read similarly to what the Claim needs) -- the resolver cannot detect this by itself; only you can.
...
- `FABRICATED_EVIDENCE`: the cited span does not contain or support the asserted information -- this includes a real, exact quote taken from the wrong section/metric/period, not only a quote that fails to appear in the report at all. Note which case it was in `notes`.
...
`source_span_start` and `source_span_end` are zero-based, end-exclusive offsets, computed exclusively by the deterministic EvidenceQuoteResolver from an exact string match against the persisted original report -- never estimated or provided by the model. For multi-span Claims, semicolon-separated offsets align by position, and `source_spans` labels every exact quote. Context is an exact, deterministic excerpt from the persisted original report, bounded to {metrics['context_char_limit_each_side']} characters on each side of each span. It is not an LLM summary.
...
def build_review_packet(
    ...
    *,
    ...
    """Build v4 artifacts atomically without mutating canonical inputs/state."""
    ...
    output_texts = {
        "phase1_human_review_v4.csv": csv_text,
        "phase1_human_review_v4.html": html_text,
        "phase1_human_review_v4_instructions.md": instructions_text,
    }
    ...
    csv_path = evaluation_dir / "phase1_human_review_v4.csv"
    html_path = evaluation_dir / "phase1_human_review_v4.html"
    instructions_path = evaluation_dir / "phase1_human_review_v4_instructions.md"
    ...
    checks = {
        ...
        in ("UNPROVEN", "UNPROVEN_V3", "UNPROVEN_V4"),
    ...
    verification = {
        ...
        "coverage": {
            **metrics,
            ...
            "review_v4_row_count": len(parsed_rows),
        },
        "checks": checks,
        "outputs": {
            "csv": str(csv_path),
        ...
        "semantic_quality": "UNPROVEN_V4",
        "human_review": "PENDING_V4",
    ...
    if _secret_findings({"phase1_human_review_packet_v4_verification.json": verification_text}):
        raise HumanReviewPacketError(
            PROVENANCE_FAILURE,
            "secret_pattern_detected_in_verification",
        )
    verification_path = evaluation_dir / "phase1_human_review_packet_v4_verification.json"
