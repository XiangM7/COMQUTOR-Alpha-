"""Phase 1 Master evidence-alignment fix: additive v3 minimal-request path
for the offline Structured Output Shadow parser.

This module never replaces or modifies ``structured_output_shadow.py`` (v1),
``structured_output_shadow_v2.py``, ``structured_output_shadow_schema.py``,
or ``structured_output_adapter.py``. It reuses, unchanged:
...
v2 proved the request could be made small; human/AI review of the v2
evaluation then found a systemic problem downstream of size: v2 asked the
Provider to compute the character offsets of its own evidence, with no
independent check that those offsets were even the right sentence -- a
Claim could be correct while its bound ``source_spans`` pointed at the
wrong part of the report. v3 changes ONLY the evidence half of the
Provider-facing proposal shape: the Provider now returns
``supporting_quotes`` (verbatim substrings of the report) instead of
numeric offsets, and this module's :class:`EvidenceQuoteResolver` (the
:func:`resolve_supporting_quote` function below) is the SOLE authority that
turns each quote into a span, by exact deterministic string match only --
never fuzzy matching, embedding search, Levenshtein repair, semantic
nearest-neighbor matching, approximate substring matching, or a second LLM
pass to fix a bad offset. A quote that cannot be found, or cannot be
uniquely located, is rejected outright, never repaired and never defaulted
to "the first match". Exact-match provenance is not semantic correctness:
this module never infers or asserts that a located quote actually supports
its claim -- that judgment remains exclusively a human review question (see
...
SHADOW_INPUT_SCHEMA_VERSION_V3 = "comqutor.structured_claim_shadow_input.v3"
SHADOW_PROVIDER_PROPOSAL_SCHEMA_VERSION_V3 = "comqutor.structured_claim_shadow_provider_proposal.v3"
...
def resolve_supporting_quote(
    *,
    ...
    to fix an offset (there is no offset to fix: v3 never accepts one from
    the model in the first place).

    - Not a non-empty string -> ``INVALID_QUOTE``.
    - Zero exact occurrences -> ``NO_EXACT_MATCH`` (a fabricated or
      malformed quote; rejected, never repaired).
...
def summarize_resolution_log(resolution_log: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    ...
    ``scripts/run_phase1_master.py``'s v3 Smoke gate ("zero fabricated
...
# ---------------------------------------------------------------------------
# Wire request -- identical compression properties to v2.
# ---------------------------------------------------------------------------
...
def select_located_candidate_hints(
    ...
    to ``structured_output_shadow_v2.select_located_candidate_hints``, but
    deliberately duplicated rather than imported: v3 must never depend on
    v2's internals drifting underneath it."""
...
def build_shadow_request_v3(
    *,
    ...
    """Build the minimal v3 wire request. Byte-for-byte the same
    compression properties as v2 (report sent exactly once; candidate
    ...
    authority-flag, no ``allowed_source_refs``). v3's Provider-facing claim
    shape differs from v2 only in swapping ``source_spans`` for
    ``supporting_quotes`` -- see the module docstring."""

    return {
        "input_schema_version": SHADOW_INPUT_SCHEMA_VERSION_V3,
        "run_id": str(run_id),
        "ticker": str(ticker),
        "agent": str(agent),
...
def normalize_v3_proposal_to_canonical_bundle(
    ...
    *,
    ...
    """Upgrade a minimal v3 Provider proposal into the exact, UNCHANGED
    ...
    canonical shape v1/v2 already produce -- never a new field, never a new
    admission path. ``resolution_log`` is a flat, bundle-EXTERNAL list of
    per-quote resolution records for Phase 1 Master's own evidence-
    ...
    on its own terms -- exactly as a malformed v1/v2 proposal would be
    rejected, not a new failure mode. A claim whose ``supporting_quotes``
    contains ANY quote that fails to resolve gets an invalid placeholder
    span (``start=None, end=None``) for that entry; ``validate_shadow_bundle``
    (unchanged) is the sole authority that then rejects the WHOLE bundle for
    it -- the same bundle-level, all-or-nothing admission v1/v2 already use,
    never a new, softer per-claim admission path.

    ``candidate_index_to_spans`` (each candidate's own located source
    spans) participates ONLY in disambiguation eligibility -- it is unioned
    with ``candidate_index_to_id`` to decide which model-referenced
    ``candidate_ids`` are "known" for that purpose, so a caller that only
    has offset hints available (no string IDs -- e.g. the Gateway-level
    pre-validation closure, which never sees full candidate metadata) can
    still disambiguate correctly; ``candidate_segment_ids`` on the emitted
    claim is still only ever populated from ``candidate_index_to_id``.
    """
...
class StructuredOutputShadowParserV3:
    """v3 sibling of ``StructuredOutputShadowParser``/``...V2``: identical
    offline, fail-soft, validator-is-sole-authority design; only the wire
    request and the Provider-facing response shape differ (verbatim quotes
    resolved deterministically instead of trusted offsets). See module
    docstring.

    ``last_resolution_log`` is reset to ``[]`` at the start of every
    :meth:`parse_report_shadow_v3` call and populated only once
    normalization actually runs (mirrors the existing
    ``Week2GatewaySemanticInvoker*.last_invocation`` side-channel pattern
    already used elsewhere in this module family) -- callers that need
    per-quote resolution diagnostics (e.g. the real-Provider smoke runner)
    read it after the call; callers that only need the bundle can ignore it
    entirely.
    """
    ...
    def __init__(
        ...
        *,
        ...
        prompt_builder: Callable[[Mapping[str, Any]], str] = build_shadow_prompt_input_v3,
        max_claims: int = DEFAULT_MAX_CLAIMS,
        max_input_bytes: int = DEFAULT_MAX_INPUT_BYTES,
        max_output_bytes: int = DEFAULT_MAX_OUTPUT_BYTES,
    ) -> None:
        if semantic_invoker is None:
            raise ValueError("semantic_invoker is required; v3 never constructs a Provider")
        ...
        *,
        ...
        try:
            ...
            return self._failure(
                ...
                status="parser_error",
                reason_code=SHADOW_JSON_INVALID,
                message=f"v3 proposal normalization failed: {type(exc).__name__}",
            )
...
def parse_report_shadow_v3(
    *,
    ...
    return parser.parse_report_shadow_v3(
        source_report=source_report,
        run_id=run_id,
        ticker=ticker,
        agent=agent,
